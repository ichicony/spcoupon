...
var config={
    user:"sp",
    password:"8766ChiconY5858",
    server:"172.28.5.203", // You can use 'localhost\\instance' to connect to named instance
    database:"spcoupon",
...
let connectionString=`mssql://${config.user}:${config.password}@${config.server}/${config.database}`
...
sql.execute=function(procedure,params,func){
    try {
        var connection = new mssql.connect(connectionString, function (error) {
            if(error)
                func(error);
            else {
...
sql.queryWithParams=function(sqltext,params,func){
    try {
        var connection = new mssql.connect(connectionString, function (err) {
            if(err)
                func(err);
            else {
                ...
                if(params){
                    for(var index in params){
                        console.log(params[index].name,params[index].value)
                        // request.input(index,params[index].sqlType,params[index].inputValue);
                        request.input(params[index].name,params[index].value);
                    }
                }
